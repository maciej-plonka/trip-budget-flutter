...
void main() {
  group("TripListCubit", () {
    ...
    blocTest("should not return state after creation",
        build: () => TripListCubit(tripService), expect: []);

    blocTest("should return two states when calling getAll() method",
        build: () {
          when(tripService.findAll()).thenAnswer((_) => Future.value([]));
          return TripListCubit(tripService);
        },
    ...
    blocTest("should return state with a list returned from service when calling getAll() method",
        build: () {
          when(tripService.findAll()).thenAnswer((_) => Future.value(listOfTripModels));
          return TripListCubit(tripService);
        },
        act: (TripListCubit cubit) => cubit.getAll(),
